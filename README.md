...
- Current best MiniMax AutoRound valid speed result is FP16 p512/n1536 on the fast-NVMe vLLM/XPU TP4 path: `41.130667` output tok/s, after a same-shape `40.864378` repeat at the full `max_model_len=2048` request window. The p512/n1024 shape reached `40.303730`, and p512/n512 reached `39.610585`. These keep normal Q/K TP allreduce, no speculative decode, and no power-limit change. LocalMaxxing accepted the p512/n1536 high as `cmoz8cow60001pd010klrb8g8`.
...
- For 4096-context capacity, `gpu_memory_utilization=0.95` is the better FP16 recipe: GPU KV cache rises to 33,408 tokens and the same p512/n1536 request improves to `36.616486` output tok/s, accepted as `cmoz8ryb9000bpd014xhl3pxu`. It still trails the 2048-window `41.130667` speed path.
...
- Removing that default-off Q/K apply+RoPE branch from the active vLLM MiniMax source restored the known-good `c15860...` AOT graph cache and recovered the runtime after the htile/rebuild work: p512/n512 returned to `38.998` output tok/s, and p512/n1536 repeated at `39.450` then `39.961` output tok/s. This is below the accepted `41.131` best, so it was not submitted to LocalMaxxing, but it is the correct active runtime state.
- A later timing-helper no-op experiment overwrote the favorable `c15860...` MiniMax AOT artifact. The current reproducible floor is back to `35-36` output tok/s after restoring the graph-shaped MiniMax source and archived `c15860...` cache, but it no longer reproduces the prior `39-41` tok/s high. Clean source without timing boundaries, source K-norm branch sync, Inductor no-autotune, and combo-kernel-off were all negative (`26.69-28.64` tok/s). Keep future compile experiments in isolated `VLLM_CACHE_ROOT` directories and treat the old `41.131` LocalMaxxing result as the published high until a deterministic faster AOT/source path is found.
...
- The first PyTorch/SYCL Level Zero IPC Q/K variance prototype now passes as a two-phase four-process test: each rank exports a PyTorch XPU mailbox tensor, opens all peer handles, writes local `[tokens,2]` Q/K variance values, synchronizes/barriers, and a SYCL kernel reads peer mailboxes to produce exact `[2.5,25.0]` averages on all ranks. The first single-kernel write+spin-read version is not correct yet because some ranks read stale sentinel values; next work is a real sequence/mailbox protocol or graph-safe two-kernel variant before vLLM integration.
...
- `notes/2026-05-10-minimax-xpu-ipc-qk-var-prototype.md`: first PyTorch/SYCL Level Zero IPC Q/K variance prototype; two-phase peer mailbox reduce passes, single-kernel ordering remains open.
