...
- MiniMax M2.7 UD-IQ4_XS now has a valid four-B70 working baseline: 13.754 tok/s for `p0/n64` using `ik_llama.cpp` process-per-GPU RPC workers with SYCL/Level Zero, layer split, runtime repack, CPU KV, fused MoE off, fused MMAD off, and a local SYCL `MULTI_ADD` implementation. This bypasses the single-process Level Zero aggregate allocation ceiling, but it is still well short of the >30 tok/s target. LocalMaxxing accepted this result as `cmovvoo6f00f5p1017yeb7kxd`.
- The next MiniMax performance blocker is fused expert execution in SYCL RPC: enabling fused MoE currently reaches `MOE_FUSED_UP_GATE`, which is not implemented in the SYCL backend. The documented next step is to port or specialize the CUDA/CPU fused up-gate path for the B70 SYCL worker, then revisit fused `MUL_MULTI_ADD` because the initial generic SYCL kernel is slower than the decomposed `mul + MULTI_ADD` path.
