...
- The PyTorch/SYCL Level Zero IPC Q/K variance prototype now passes both the two-phase four-process mailbox test and a single-kernel sequence-counter stress test. Each rank exports PyTorch XPU mailbox tensors, opens all peer handles, writes local `[tokens,2]` Q/K variance values, publishes a sequence counter, polls peer counters, and reads peer payloads from one SYCL kernel. The sequence path passed 4 ranks, 50 iterations, and 32 token rows with exact `[51.5,515.0]` final averages. The naive float-sentinel single-kernel variant remains a negative result because some ranks read stale sentinels.
...
- `notes/2026-05-10-minimax-xpu-ipc-qk-var-prototype.md`: PyTorch/SYCL Level Zero IPC Q/K variance prototype; two-phase peer mailbox reduce passes and the single-kernel sequence-counter path now passes 4-rank stress.
