...
Note: `cmouvurhh00nqld010dtr4xrl` is the first submitted root-residual quality-preserving Q4_0 GGUF result. It adds `GGML_SYCL_COMM_FUSEADD_ROOT_RESIDUAL=1` to the guard-fix stack, reusing the mirrored root residual in fused allreduce+ADD instead of reading peer residual buffers. The run keeps the same Q4_0 weights, f16 KV cache, flash attention, Q8 activation cache, fused MMVQ2, fused MMVQ2+SwiGLU, fused RMS_NORM+MUL, fused allreduce+ADD, fused final GET_ROWS, single-kernel allreduce, no speculative decoding, no sampling change, and no power-limit change. An initial `llama-cli` text smoke was inconclusive, but a later capped deterministic `llama-completion` byte-compare matched baseline stdout exactly with non-empty output.

Note: `cmouxjqao000npn01hxqn68td` is the current best submitted quality-preserving Q4_0 GGUF result. It keeps the same TP3 root-residual stack as `cmouvurhh00nqld010dtr4xrl` and changes only `--poll 50` to `--poll 25`. A short poll sweep favored `25`, and the full validation produced `50.922114 tok/s` output and `81.243035 tok/s` total. The detailed annotated LocalMaxxing payload returned HTTP 500, while the reduced core-metric payload was accepted.
