...
LocalMaxxing accepted the first p512/n512 control run as `cmoz7rs2w0077tl01o3f1kxnm`.
...
The valid p512/n512 control repeated tightly at `39.61` then `39.52` output tok/s. This uses the normal Q/K TP allreduce, normal MiniMax math, the same AutoRound W4A16 target weights, no speculative decode, and no power-limit change. The likely contributors versus older p512/n512 records are the fast NVMe setup, installed B70 MoE config, current warmed compile/cache state, and the stable `max-num-batched-tokens=1024` scheduler shape.
...
- Run p512/n1024 again with the same warmed state to see whether the 39 tok/s behavior holds over longer decode, or whether the ceiling falls back to the previous 36-37 tok/s long-output range.
