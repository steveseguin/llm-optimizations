...
- Layer-mode follow-up screens were negative: `-t` sweep topped out at `16.307 tok/s`, `-fa 1` aborts on unsupported `FLASH_ATTN_EXT`, disabling fused MMAD/MoE is slower at p0/n64, same-type contiguous copy memcpy is neutral, and an 8-expert `MUL_MULTI_ADD` unroll regressed to `13.823 tok/s` and was removed.
