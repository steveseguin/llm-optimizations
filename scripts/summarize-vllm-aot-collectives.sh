...
count_rg "^[[:space:]]*torch\\.ops\\._c10d_functional\\.all_reduce_?\\.default"
printf 'wait_tensor_call_lines='
count_rg "^[[:space:]]*torch\\.ops\\._c10d_functional\\.wait_tensor\\.default"
...
rg -n "Topologically Sorted Source Nodes: \\[all_reduce|PlaceHolder\\[target=all_reduce|^[[:space:]]*torch\\.ops\\._c10d_functional\\.all_reduce_?\\.default|^[[:space:]]*torch\\.ops\\._c10d_functional\\.wait_tensor\\.default|torch\\.ops\\.vllm\\.all_reduce|rms_norm_default, int4_gemm_w4a16|Original ATen: \\[aten\\._to_copy, aten\\.add, aten\\.pow, aten\\.mean, aten\\.rsqrt, aten\\.mul, vllm\\.moe_forward\\]" "$cache" -g '*.py' 2>/dev/null | head -160 || true
