...
1. vLLM AutoRound INT4:
   ...
   - Continue `llm-scaler` custom ESIMD INT4 MoE work, but do not enable the current Python-level vLLM integration for normal p512 runs. The exact MiniMax TP4 routed MoE microbench (`hidden_size=3072`, `intermediate_size=384`, `num_experts=256`, `top_k=8`) is strongly positive: bs=1 improves from `355.7 us` to `91.1 us`, and bs=64 improves from `3132.9 us` to `1033.8 us`. Decode isolation is also positive: p1/n128 with `VLLM_XPU_USE_LLM_SCALER_MOE=1` reached `34.06` output tok/s versus `22.31` without it, LocalMaxxing `cmoxofv8h00h5ml01wu1uakhr`. The full p512/n128 integration is negative: `12.27` output tok/s with llm-scaler versus `20.17` without it. Next useful work is a BF16-capable tiny kernel, an unsigned-weight decode-only variant, or a monolithic C++/SYCL N-major routed op that avoids Python route/gather overhead.
