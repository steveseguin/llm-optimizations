...
1. vLLM AutoRound INT4:
   ...
   - Treat GPU `ngram_gpu` speculative decode as negative for this harness. It kept async scheduling but reached only `3.15` output tok/s on p64/n16.
   ...
   - Continue `llm-scaler` custom ESIMD INT4 MoE work, but do not enable the current Python-level vLLM integration for normal runs. The exact MiniMax TP4 routed MoE microbench (`hidden_size=3072`, `intermediate_size=384`, `num_experts=256`, `top_k=8`) is strongly positive: bs=1 improves from `355.7 us` to `91.1 us`, and bs=64 improves from `3132.9 us` to `1033.8 us`. The first full-model integration is negative: p512/n128 with `dtype=float16` and `VLLM_XPU_USE_LLM_SCALER_MOE=1` reached only `61.374542` total tok/s and `12.27` output tok/s, versus `100.832219` total and `20.17` output for the FP16 baseline without llm-scaler. Next useful work is a BF16-capable tiny kernel, an unsigned-weight decode-only variant, or a monolithic C++/SYCL N-major routed op that avoids Python route/gather overhead.
...
- llm-scaler ESIMD INT4 MoE exact-shape microbench is positive, but the first opt-in vLLM integration is negative at full-model p512/n128: `12.27` output tok/s with `VLLM_XPU_USE_LLM_SCALER_MOE=1` versus `20.17` output tok/s for the FP16 baseline without it. Keep the patch as a reproducibility artifact, not a default.
