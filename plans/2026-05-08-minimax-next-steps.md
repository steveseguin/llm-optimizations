...
1. vLLM AutoRound INT4:
   ...
   - Treat `MirecX/MiniMax-M2.7-L3H5-DFlash` as blocked/negative on the current TP4 XPU stack. The DFlash drafter loads, compiles, shares target embeddings/lm head, and selects taps `(2, 16, 30, 43, 57)`, but a p64/n16 smoke stalls at the first request, logs a shared-memory broadcast wait, and produces no JSON throughput. The drafter's own card reports `m_accept ~= 1.38`, already below the expected speed break-even, and it targets an AWQ checkpoint rather than the AutoRound checkpoint. Do not retest as a speed path without first isolating the speculative-loop stall on a smaller target or with `num_speculative_tokens=1`.
...
- vLLM DFlash speculative decode with `MirecX/MiniMax-M2.7-L3H5-DFlash` loaded and compiled but stalled before completing a p64/n16 request. No JSON throughput was produced, so it was not submitted.
